...
{ (float) width, (float) length, 6.0, 5.0 };
float lightPosition2[] =
{ 0, (float) length, 6.0, 5.0 };
float lightPosition3[] =
{ 0, 0, 6.0, 5.0 };
float lightPosition4[] =
{ (float) width, 0, 6.0, 5.0 };
float light[] =
{ 0.8, 0.8, 0.8, 0.7 };
float environment[] =
{ 0.9, 0.6, 0.6, 1.0 };
...
bool goForwardIn()
{
	int x = (int) (((vettori.eye[0] + s + 1) / c) + l * cos((a * 3.14) / 180));
	int y = (int) (((vettori.eye[1] + s + 1) / c) + l * sin((a * 3.14) / 180));
	if (map[x][y] == 1)
	{
...
void positionRotatingCube(int i, int j)
{
	glPushMatrix();
	glColor3f(1, 0, 0);
	glTranslatef((i * c) + s, (j * c) + s, s);
	glRotatef(rotate, 0, 0, s);
	glutSolidCube(d);
	glPopMatrix();
	glColor3f(0, 1, 0);
}
